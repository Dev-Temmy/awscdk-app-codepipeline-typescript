...
export class FargateDemoStack extends cdk.Stack {
    ...
    // VPC
    const vpc = new Vpc(this, "fargatecdkstackVPC", {
        maxAzs: 2,
        natGateways: 1,
    });

    // Fargate cluster
    const cluster = new ecs.Cluster(this, "fargatecdkstackCluster", {
        vpc: vpc as any,
    });
    ...
    const backendService = new ecs_patterns.ApplicationLoadBalancedFargateService(this, "backendService", {
        ...
        desiredCount: 2,
        taskImageOptions: {
        image: ecs.ContainerImage.fromAsset("../aws_cdk_stack_typescript/backend/"),
        environment: {
            myVar: "variable01",
        },
        },
